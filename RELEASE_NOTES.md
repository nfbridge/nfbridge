# Neo Film Bridge v0.7.1 프리뷰 — Mac 앱 후보

첫 연결 검사에서 서로 다른 디스크의 표시 이름이 같으면 앱이 이를 오류로 처리하던 문제를 고쳤습니다. 검사에 실패하면 이제 권한 문제라고 단정하지 않고 실패한 항목을 표시합니다. 수정판을 추가 Mac에서 재시험해 F100 촬영정보 1롤·12컷을 GUI로 가져오고 저장했으며, 생성된 HTML 표도 열어 확인했습니다.
