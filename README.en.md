...
The source code is in this repository. Download the prebuilt Apple Silicon Mac app ZIP from [Releases](https://github.com/nfbridge/nfbridge/releases). It unzips to a folder containing the app, guides and license notices. Start with `START_HERE.en.md`.
...
The connection path is **Nikon F100 → Nikon MC-31 (the factory RS-232 data cable) → a supported Serial-to-USB adapter → Mac**. macOS and this app observe the adapter as the USB device; the MC-31 itself is not a USB device. The cable check recognizes Serial-to-USB adapters from **two vendor families: Prolific (vendor ID `067b`) and FTDI (vendor ID `0403`)** — product IDs are not pinned to a specific value within either vendor, since the admission pipeline's actual security boundary is fingerprint sealing, topology binding and explicit user confirmation, not the vendor match itself. The FTDI path has completed physical testing — import and erase — with a real F100 and MC-31 using an FTDI-based Serial-to-USB adapter (`0403:6001`, FT232R family) as of 2026-09-17. Widening beyond Prolific 067b:2303 to other Prolific product IDs has not yet been physically tested, only verified with offline tests. Even once an adapter's chipset is recognized, a first-time adapter still requires you to confirm it physically through the "Re-check cable" flow.
