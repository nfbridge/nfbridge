...
## F100 flash field semantic correction and closure (2026-09-19)

Real F100 hardware records from two rolls (real-only shooting, read through the existing read-only `lq` import path, no writes) showed that `f100_readonly.py::decode_lq()`'s flash-related labels were wrong or incomplete. This is a decode-semantics correction to an existing field, not a new feature; the record structure, byte offsets, and JSON schema are unchanged.

**Real evidence**: Roll 47 (11 detailed frames) and Roll 48 (6 detailed frames), captured together in one `F100Capture` live-com-observe session (`20260919-002722-CameraCompanion-COM3`) and independently cross-checked frame-by-field against Camera Companion's own decoded DB export for the same two rolls (`flashdata_Frames.csv` / `flashdata2_Frames.csv`). Both evidence sources are held outside this repository, in the private research archive, per this project's publication policy; the minimal 13-byte records needed to reproduce the finding are embedded directly in `mac-client/test_flash_decode.py`'s `RealRecordRegressionTests`, with the source session/export SHA-256 values cited in that file's module docstring.

**FLASH TYPE USER-STATE DOMAIN: CLOSED** — `rec[4] & 0x03`: `0` = Off, `1` = Non-TTL, `3` = TTL. (`2` remains reserved/unobserved.) The mask itself is also corrected, from the previous `rec[4] & 0xEF` (which left the value entangled with unrelated/reserved bits and silently misclassified any record with a stray high bit set as `unknown`, even when its low two bits were a valid code) to the structurally correct `rec[4] & 0x03`; `multiple_exposure` (bit `0x10`) is confirmed independent and is not affected.

**FLASH SYNC USER-STATE DOMAIN: CLOSED** — `(rec[12] >> 4) & 0x07`: `0` = Normal, `1` = Slow, `2` = Rear, `3` = Red-eye, `4` = Red-eye + Slow. Real Roll 48 evidence anchors code `3` = Red-eye directly (frame 2, `rec[12]=0x38`); Roll 47 anchors `0`/`1`/`2`/`4`. `rec[12]` bit 7 is confirmed not part of this field and remains unlabeled.

**FLASH COMPENSATION: CLOSED** — `rec[11]`, unchanged: `signed(code)/6` EV, the same generic `"ev"` decode domain already shared with `manual_ev_diff`/`exposure_comp`. Real anchors `0x00`→`0.0`, `0x06`→`+1.0`, `0xFA`→`-1.0` matched the existing formula exactly; no decode change was needed here, only confirmation.

**MULTIPLE EXPOSURE FLAG: CLOSED** — `rec[4] & 0x10`, confirmed independent of flash type across real combinations `0x03` (TTL, ME off), `0x13` (TTL, ME on), `0x10` (flash off, ME on).

**RESERVED RAW CODES** — flash type `2` and flash sync `5`/`6`/`7` are intentionally left `unknown(0x..)`, not guessed or promoted to a label. Whether the real F100 ever generates these codes under any operation is not established by this closure; normal user-facing flash states (Off/Non-TTL/TTL × Normal/Slow/Rear/Red-eye/Red-eye+Slow) were all directly observed, but this is not a claim that every 8-bit raw combination has been exercised.

**Past static-analysis label errors (recorded here, not corrected retroactively in their original documents, which live outside this repository)**: an earlier static-analysis pass had flash type `1`/`3` swapped (labeling `1` as TTL and `3` as Non-TTL) and flash sync `Slow`/`Rear` swapped. Both were structural/bit-position work, correctly identifying which bits carry these fields — only the semantic label assigned to each code value was wrong, and this closure corrects the label mapping in `f100_readonly.py`, not the bit positions (which were already right).

Regression coverage: `mac-client/test_flash_decode.py` (19 tests: synthetic domain tables for flash type/sync/multiple-exposure/flash-comp, raw preservation, and 12 real Roll 47/48 record cases), `mac-client/test_transport_regressions.py::test_flash_type_is_bits_1_0_only_and_ignores_other_bits` / `::test_flash_type_reserved_code_2_stays_unknown`, `tests/test_exports.py::test_flash_display_covers_non_ttl_and_all_real_sync_labels` / `::test_report_rows_render_non_ttl_and_sync_labels`, and the existing `mac-client/fixtures/golden/detailed_two_rolls` golden-parity fixture (whose own `flash_type` expectation for a `0x11`-flagged synthetic record was itself corrected from `unknown(0x01)` to `non_ttl`, since that record's low two bits were always a valid Non-TTL code under the corrected mask). Camera transport, DP/NP/EP, USB/serial binding, connection policy, erase/settings policy, GUI structure, packaging, and release/tag process were not touched.
